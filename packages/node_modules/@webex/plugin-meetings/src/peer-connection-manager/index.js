      ...
      if (!validateSdp(peerConnection.sdp)) {
        setTimeout(() => {
          ...
          if (validateSdp(peerConnection.sdp)) {
            ...
            reject(new MediaError('No ice candidate or port found in the SDP'));
          }
        }, RETRY_TIMEOUT);
      }
      else {
...
pc.setRemoteSessionDetails = (peerConnection, typeStr, remoteSdp, meetingId) => {
  ...
  const opusRtpmap = EXTRACT_OPUS.exec(sdp);

  if (opusRtpmap && opusRtpmap.length > 0) {
    sdp = sdp.replace(opusRtpmap[0], `${opusRtpmap[0]}a=fmtp:${opusRtpmap[1]} ${INLINEFEC}\r\n`);
  }

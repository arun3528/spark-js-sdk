...
import {InvalidSdpError} from '../util/webex-errors';
...
const validateSdp = (sdp) => {
  ...
  parsedSdp.media.forEach((mediaLine) => {
    if (mediaLine.candidates && mediaLine.candidates.length === 0) {
      ...
      validSdp = validSdp && false;
      errorMessage = 'iceCandidate: Ice candadate never completed';
    ...
    if (hostCandidate.length === 0) {
      LoggerProxy.logger.error('PeerConnectionManager:index#validateSdp --> iceCandidate: no IPV4 candidate present');
      validSdp = validSdp && false;
      errorMessage = 'iceCandidate: no IPV4 candidate present';
    ...
    if (SDP.BAD_MEDIA_PORTS.includes(mediaLine.port)) {
      ...
      validSdp = validSdp && false;
      errorMessage = 'iceCandidate: Found invalid port number for the ice candidate';
    ...
    if (!mediaLine.icePwd || !mediaLine.iceUfrag) {
      ...
      validSdp = validSdp && false;
      errorMessage = 'iceCandidate: ice ufrag and password not found';
    }
  });

  return {validSdp, errorMessage};
};
...
pc.iceCandidate = (peerConnection, {remoteQualityLevel}) =>
  new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (!validateSdp(peerConnection.sdp).validSdp) {
        setTimeout(() => {
          ...
          const validationResult = validateSdp(peerConnection.sdp);

          if (validationResult.validSdp) {
            resolve(peerConnection);
          }
          else {
            LoggerProxy.logger.error('PeerConnectionManager:index#iceCandidate --> SDP not valid after waiting.');
            reject(new InvalidSdpError(validationResult.errorMessage));
          }
        }, RETRY_TIMEOUT);
      }
      else {
    ...
    peerConnection.onicecandidate = (evt) => {
      if (!evt.candidate && !peerConnection.sdp) {
        ...
        if (evt.candidate === null && validateSdp(peerConnection.sdp)) {

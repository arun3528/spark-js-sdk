...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  setUpLocusInfoSelfListener() {
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.MEDIA_STATUS_CHANGE, (status) => {
      if (this.statsAnalyzer) {
        console.log('MEDIA STATS COMPARISION ', {
          ...
          sendShare: this.mediaProperties.mediaDirection?.sendShare && this.isSharing,
  ...
  muteAudio() {
    ...
    if (!this.mediaId) {
      // Happens when addMedia and mute are triggred in succession
      return Promise.reject(new MediaError('media connection has not been established'));
    }
  ...
  unmuteAudio() {
    ...
    if (MeetingUtil.isUserUnjoined(this.locusInfo)) {
      return Promise.reject(new MediaError('User cannot perform action as already in a LEFT state'));
    }
  ...
  addMedia(options = {}) {
    ...
    if (this.meetingState !== MEETING_STATE.ACTIVE) {
      return Promise.reject(new MediaError('Meeting needs to be active to add media'));
    }

    // If the user is unjoined or guest waiting in lobby dont allow the user to addMedia
    if (MeetingUtil.isUserUnjoined(this.locusInfo) && !this.wirelessShare) {
  ...
  leave(options = {}) {
    ...
    if (this.meetingState === MEETING_STATE.INACTIVE) {
      // TODO: clean up if the meeting is already inactive
      return Promise.reject(new MediaError('Meeting is already in a END STATE'));
    }

...
import {MeetingNotActiveError, UserNotJoinedError} from '../util/webex-errors';
...
MeetingUtil.isUserInLeftState = (locusInfo) =>
  locusInfo.parsedLocus && locusInfo.parsedLocus.self && locusInfo.parsedLocus.self.state === _LEFT_;

MeetingUtil.isUserInIdleState = (locusInfo) =>
  locusInfo.parsedLocus && locusInfo.parsedLocus.self && locusInfo.parsedLocus.self.state !== _IDLE_;
MeetingUtil.isUserInJoinedState = (locusInfo) =>
  locusInfo.parsedLocus && locusInfo.parsedLocus.self && locusInfo.parsedLocus.self.state === _JOINED_;

=======
MeetingUtil.isUserUnjoined = (locusInfo) =>
  locusInfo.parsedLocus && locusInfo.parsedLocus.self && locusInfo.parsedLocus.self.state !== _JOINED_;
>>>>>>> fix(plugin-meetings):  throw error when meeting is inactive on a action
